...
stocks = st.text_input("Enter the stocks in your portfolio, separated by commas (e.g., 'SNOW, CRM, MSFT')", key='stocks_input').split(',')
amounts = st.text_input("Enter the corresponding amounts of each stock in your portfolio, separated by commas (e.g., '100, 200, 300')", key='amounts_input').split(',')
...
if next_button.button("Next"):
    ...
    st.session_state.scenario_index = min(len(scenario_objects) - 1, st.session_state.scenario_index + 1)
...
if st.button('Start Game'):
    ...
    for scenario_name in st.session_state.selected_scenarios:
        scenario = next(scenario for scenario in scenario_objects if scenario.name == scenario_name)
